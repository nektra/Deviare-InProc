...
DEVENV vs2008\NktHookLib.sln /rebuild "Release|Win32" /project "DeviareLiteInterop" 
IF NOT %ERRORLEVEL% == 0 goto bad_compile
ENDLOCAL
...
REM DeviareLiteInterop depends on DeviareLiteCOM
REM DeviareLiteCOM depends on NktHookLib
DEVENV vs2008\NktHookLib.sln /rebuild "Release|x64" /project "DeviareLiteInterop"
IF NOT %ERRORLEVEL% == 0 goto bad_compile
ENDLOCAL
GOTO end
